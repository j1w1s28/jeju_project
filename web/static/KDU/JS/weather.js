...
var outerSVG = Snap('#weatherOuter');
...
function onResize() {
    ...
    // leafMask.attr({ x: sizes.card.offset.left, y: 0, width: 810 + sizes.container.width - sizes.card.offset.left, height: sizes.container.height });
}
...
$.ajax({
    ...
    success: function (data) {
        ...
        document.querySelector('.temp').innerHTML = Math.round(data.main.temp - 273.15) + '<span>ºC</span>';
    },
});

//날씨 checker
function weatherCheck(wd) {
    // 뇌우
    if (wd >= 200 && wd <= 232) changeWeather(weather[3]);
    // 비
    else if (wd >= 300 && wd <= 531) changeWeather(weather[2]);
    // 눈
